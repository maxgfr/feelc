...
Embeds `@feelc/engine` in a Cloudflare Worker. Edge runtimes have no `node:fs` and don't resolve
...
```bash
# from the repo root, after `npm install` and `npm -w @feelc/engine run build`
...
The same pattern works in Deno (`import wasm from "@feelc/engine/wasm/feelc.wasm"` then
